...
bool Math::GetCirclesIntersect(float X1, float Y1, float R1, float X2, float Y2, float R2, FPoint2D &P1, FPoint2D &P2)
{
	...
	// »щем пересечение ближайшее только к Near1.
	float  C1 = R1 * R1 - R2 * R2 + Y2 * Y2 - (X2 - X1) * (X2 - X1) - Y1 * Y1,  // замен€юща€ константа.
		C2 = X2 - X1,                                                        // замен€юща€ константа.
		C3 = Y2 - Y1,                                                        // замен€юща€ константа.
		a  = -4 * C2 * C2 - 4 * C3 * C3,
	...
	{
		assert(false);
		if ( X1 == X2  &&  Y1 == Y2  &&  R1 == R2 )  // ≈сли окружности совпадают, то будем считать, что точка тут:
		{
			P1.x = X1 - R1;
			P1.y = Y1;
			P2.x = X1 + R1;
