...
//
MyAnimation::MyAnimation(TiXmlElement *xe, HTEXTURE hTexture)
{
	...
	while (element) {
		_bones.push_back(new MyMovingPart(element, hTexture));
...
//
void MyAnimation::SetPos(const hgeVector &pos, bool mirror) {
	_subPosition.Unit();
	_subPosition.Move(-_pivotPos.x, -_pivotPos.y);
	if (mirror) {
...
//
void MyAnimation::Draw(float position) {
	_matrixsStack.clear(); 
	...
	if (doc.LoadFile(hge->Resource_MakePath(fileName.c_str()))) { // если файл существует - читаем XML с описанием анимаций
		...
		while (animation) {
			...
			if (_animations.find(id) == _animations.end()) { 
				_animations[id] = new MyAnimation(animation, tex);
			}
...
//
MyAnimation *AnimationManager::getAnimation(const std::string &animationId) {
